...
import build.log;
import build.platform :objName, libName, exeName;
import build.proc : takeEnvArgs;

import makelib.engine;
import makelib.filesystem;
...
`, res.options);
    ...
    engine.rule()
        .target(buildPath(env["G"], "lexer").libName)
        //.depends(sources.lexer)
        //.depend(configFiles)
        .action(delegate(Item target, Target[] targets, Item[] deps)
        {
            run([env["HOST_DMD_RUN"], "-of$@", "-lib",
                "-J"~env["G"], "-J../res"].chain(flags["DFLAGS"], sources.lexer).array);
        });
...
/// Returns: the rule that builds and executes the optabgen utility
auto opTabGen()
{
...
version(Windows)
{
    /// Returns: the rule that builds msvc-dmd.exe
    auto buildMsvcDmc()
    {
    ...
    /// Returns: the rule that builds msvc-lib.exe
    auto buildMsvcLib()
    {
...
/**
Gets the rule that generates the given object file from the given source file

Params:
    obj      = the object file that the rule should generate
    fileName = the source file to build, generating the object file
Returns:
    the rule that generates the given object file from the given source file
*/
...
auto buildBackend()
{
    ...
    BuildRule[] dependencies = cxxBackend();
    dependencies ~= dBackend;
    foreach (rule; dependencies.parallel(1))
        rule.run;
...
{
    const versionFile = buildPath(env["G"], "VERSION");
    auto commandFunction = (BuildRule* rule){
        "(TX) VERSION".writeln;
        ["git", "describe", "--dirty"].runCanThrow.toFile(versionFile);
    };
    BuildRule versionRule = {
        targets: [versionFile],
    ...
    BuildRule sysconfDirRule = {
        sources: [thisBuildScript],
        target: sysconfDirFile,
        commandFunction: commandFunction,
    };
    return [versionRule, sysconfDirRule];
}
...
Defines the required order for the build dependencies, runs all these rule dependencies
...
auto buildDMD(string[] extraFlags...)
{
    version(Windows)
    {
        ...
        if (model == "64")
        {
            foreach (rule; [buildMsvcDmc, buildMsvcLib].parallel(1))
                rule.run;
        }
    }

    // The string files are required by most targets
    BuildRule[] dependencies = buildStringFiles();
    foreach (rule; dependencies.parallel(1))
        rule.run;

    dependencies = [lexer, dmdConf];
    foreach (ref rule; dependencies.parallel(1))
        rule.run;
...
auto run(T)(T args)
{
    logf("Run: %s", escapeShellCommand(args));
